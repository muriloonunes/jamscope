...
class FriendGroupWidget : GlanceAppWidget() {
    ...
    private fun ProfileImage(
        ...
        imageBitmap?.let {
            Image(
                ...
                modifier = GlanceModifier.cornerRadius(50.dp).size(50.dp)
            )
        } ?: run {
            Image(
...
fun Context.startGroupUpdateWorker() {
    val oneTimeRequest = OneTimeWorkRequest.Builder(FriendGroupWidgetWorker::class.java)
...
fun Context.startPeriodicWorker() {
    val periodicRequest =
        PeriodicWorkRequest.Builder(FriendGroupWidgetWorker::class.java, 15, TimeUnit.MINUTES)
            .setBackoffCriteria(
